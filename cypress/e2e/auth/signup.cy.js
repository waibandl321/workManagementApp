describe('サインアップ画面', () => {
    it('画面遷移', () => {
        ...
        cy.get('[data-e2e-id="googleSignup"]')
        cy.get('[data-e2e-id="pageMoveSignin"]').should('contain', 'ログインはこちら')
    })
    ...
    it('アカウント登録成功', () => {
        ...
        cy.get('[data-e2e-id="execSignup"]').should('not.have.attr', 'disabled')
        // ログイン処理待ち
    ...
    it('アカウント重複', () => {
        cy.visit('/auth/signup')
        cy.get('[data-e2e-id="inputEmail"]').clear().type("e2etest@example.com")
        cy.get('[data-e2e-id="inputPassword"]').clear().type("e2eteste2etest")
        cy.get('[data-e2e-id="execSignup"]').should('not.have.attr', 'disabled')
        // ログイン処理待ち
        cy.get('[data-e2e-id="execSignup"]').click()
        cy.wait(2000)
    ...
    it('サインイン画面遷移', () => {
        cy.get('[data-e2e-id="pageMoveSignin"]').click()
        cy.wait(500)
