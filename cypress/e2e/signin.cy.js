describe('サインイン画面', () => {
  it('画面遷移', () => {
    ...
    cy.get('[data-ut-id="execSignin"]').should('have.attr', 'disabled', 'disabled')
    cy.get('[data-ut-id="passwordReset"]').should('contain', 'パスワード再設定')
    cy.get('.v-card__title').should('contain', '外部サービスでサインイン')
    cy.get('[data-ut-id="googleSignin"]')
    cy.get('[data-ut-id="pageMoveSignup"]').contains('ユーザー登録がまだの方はこちら')
  })
  ...
  it('認証エラー', () => {
    // メールアドレス不一致
    cy.get('[data-ut-id="inputEmail"]').clear().type("e2etest@example.co")
    cy.get('[data-ut-id="inputPassword"]').clear().type("e2eteste")
    cy.get('[data-ut-id="execSignin"]').should('not.have.attr', 'disabled')
    cy.get('[data-ut-id="execSignin"]').click()
    cy.wait(2000)
    ...
    cy.get('[data-ut-id="inputPassword"]').clear().type("e2etestx")
    cy.get('[data-ut-id="execSignin"]').should('not.have.attr', 'disabled')
    cy.get('[data-ut-id="execSignin"]').click()
    cy.wait(2000)
    cy.get('.v-alert').should('contain', '認証に失敗しました。正しいメールアドレス、パスワードを入力してください。')
  ...
  // 遷移OKだけ確認
  it('パスワード再設定画面遷移', () => {
    cy.get('[data-ut-id="passwordReset"]').click()
    cy.wait(500)
  ...
  it('サインアップ画面遷移 ', () => {
    cy.visit('/auth/signin')
    cy.get('[data-ut-id="pageMoveSignup"]').click()
    cy.wait(500)
